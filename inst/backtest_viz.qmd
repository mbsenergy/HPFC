...
dt_pwr_fwd = readRDS(file = file.path('new_forecast.rds'))
dt_pwr_fwd[, type := 'FOR']
dt_pwr_fwd = dt_pwr_fwd[, .(date, hour, RIC, value = final_forecast, type)]

# Spot
dt_pwr_spot = HPFC::dt_spot_pwr[year(date) %in% unique(year(dt_pwr_fwd$date)) & RIC == unique(dt_pwr_fwd$RIC)]
dt_pwr_spot = dt_pwr_spot[, .(date, hour, RIC, value = smp, type = 'OBS')]
...
### Error ANOVA
**Leven Test:**
If **Levene’s test** shows **heterogeneity of variance**, ANOVA assumptions are violated. Standard ANOVA is unreliable in this case.

**Kruskal-Wallis test:**
Unlike ANOVA, the **Kruskal-Wallis test (a non-parametric test) does not assume normality and is robust to heteroskedasticity**.
The significant p-value suggests at least one season has a different error distribution.

**Wilcoxon rank sum test:**
The Wilcoxon rank sum test shows significant differences in error distributions between most seasons.
```{r, error-correlation}
dt_pwr_error_anova = dt_pwr_error[!is.na(ERROR)]

levene_result = car::leveneTest(ERROR ~ factor(season), data = dt_pwr_error_anova)
knitr::kable(as.data.frame(levene_result), caption = "Levene's Test for Homogeneity of Variance")

kruskal_result = kruskal.test(ERROR ~ factor(season), data = dt_pwr_error_anova)

# Create a formatted table for Kruskal-Wallis Test
knitr::kable(data.frame(Statistic = kruskal_result$statistic, 
                 DF = kruskal_result$parameter, 
                 P_value = kruskal_result$p.value), 
      caption = "Kruskal-Wallis Test Results")


pairwise_result = pairwise.wilcox.test(dt_pwr_error_anova$ERROR, 
                                        dt_pwr_error_anova$season, 
                                        p.adjust.method = "bonferroni")

# Convert pairwise test results to a data frame
pairwise_table = as.data.table(pairwise_result$p.value)
pairwise_table = melt(pairwise_table, variable.name = "Comparison", value.name = "P_value")


# Create a formatted table for Pairwise Wilcoxon Test
knitr::kable(pairwise_table, caption = "Pairwise Wilcoxon Test Results (Bonferroni Adjusted)")
