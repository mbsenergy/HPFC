...
hourly_regressors=function(dataframe){
    ...
    } else if (!('hour' %in% colnames(dataframe)) | class(dataframe$hour) != 'numeric') {
        stop("hour column must be format numeric")
    } else if (!('smp' %in% colnames(dataframe)) | class(dataframe$smp) != 'numeric') {
        stop("smp column must be format numeric")
    } else if (!('break_group_p' %in% colnames(dataframe)) | class(dataframe$break_group_p) != 'integer') {
        stop("break_group_p column must be format integer")
    }
    ...
    # Calculate SMP deviation from baseload
    filtered_dam_ddhh[, smp_h := smp - bl]
    
    # Polynomial terms for baseload
