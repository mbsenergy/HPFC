...
#
# free_forward_power=readRDS(file = file.path('..', 'HPFC','data', 'data_package', "free_fwd_power.rds"))
#
# free_forward_gas=readRDS(file = file.path('..', 'HPFC','data', 'data_package', "free_fwd_gas.rds"))
#
# calendar_daily=readRDS(file = file.path('..', 'HPFC','data', 'data_package', "calendar_daily.rds"))
#
# history = read.xlsx(file.path('..', 'HPFC','data', '1_data_raw', "input_part_1", "DAM data.xlsx"), sheet = paste0('GR', "_DAM")) |> setDT()
...
# calendar_daily = history_day[calendar_daily, on = 'date']                 # merge
#
# #### merge calendar with forward
# calendar_daily = free_forward_power[calendar_daily, on = c('month', 'year')]        # merge
#
# #### merge calendar with forward
# calendar_daily = free_forward_gas[calendar_daily, on = c('month', 'year')]        # merge
#
# last_date=as.Date(max(history$date))
# #### spot before current date and fwd after for BL
# calendar_daily[, spot_forward_month_BL := fifelse(date <= last_date, smp_day, BL_quotes)]
# #### NA before current date and fwd after for PL
# calendar_daily[, spot_forward_month_PL := fifelse(date <= last_date | PL_quotes <= 0, as.numeric(NA), PL_quotes)]
#
# long_term_param=readRDS(file = file.path('..', 'HPFC','data', 'data_package', "long_term_param.rds"))
#
#
# LT_calibration=function(dataframe,profile_matrix){
#
...
#
#
# f3=LT_calibration(calendar_daily,long_term_param)
# saveRDS(f3, file = file.path('..', 'HPFC','data', 'data_package', "dd_Lt_day.rds"))
#
